...
class ShiftTrackerApp {
                            ...
                            <div style="display: flex; align-items: center; margin-bottom: var(--spacing-sm); border-bottom: 2px solid rgba(${accent.rgb}, 0.2); padding-bottom: 8px;">
                                ...
                                <div class="group-actions">
                                    ...
                                    ${!allCompleted && completedCount > 0 ? `
                                        <button class="btn-group-action" data-action="clear-completed" data-project="${SecurityUtils.escapeHtml(project)}" title="Clear all completed items">
                                    ...
                                    ${allCompleted ? `
                                        <button class="btn-group-action" style="color: var(--success);" data-action="archive-group" data-project="${SecurityUtils.escapeHtml(project)}" title="Archive completed list">
                            ...
                            <div class="list-items-stack" style="display: flex; flex-direction: column; gap: 8px;">
                                ${items.map((item, index) => `
                                    <div class="list-item animate-in delay-${Math.min(index, 19)} ${item.isCompleted ? 'completed' : ''}" draggable="true" data-item-id="${item.id}">
                                        <div style="display: flex; align-items: center; gap: 12px; flex: 1; cursor: pointer; padding-right: var(--spacing-sm);" data-action="toggle-item" data-id="${item.id}">
                                        ...
                                        <button class="list-item-delete" data-action="delete-item" data-id="${item.id}" title="Delete item">
    ...
    switchTab(tabName) {
        ...
        // Close any active modals (like Add Shift) when switching tabs
    ...
    setupEventListeners() {
        ...
        this.elements.tabs.forEach(tab => {
            ...
            tab.addEventListener('keydown', (e) => {
                const tabs = [...this.elements.tabs].filter(t => t.hasAttribute('data-tab'));
        ...
        if (this.elements.listsContainer) {
            this.elements.listsContainer.addEventListener('click', (e) => {
                ...
                switch (action) {
                    case 'toggle-item':
                        if (!isNaN(id)) this.toggleListItem(id);
                        break;
                    case 'delete-item':
                        if (!isNaN(id)) this.deleteListItem(id);
                        break;
                    case 'clear-completed':
                        if (project) this.clearCompleted(project);
                        break;
                    case 'archive-group':
                        if (project) this.deleteProjectGroup(project);
                        break;
                }
            });
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if ('serviceWorker' in navigator) {
        const swCode = `
const CACHE_NAME = 'shift-tracker-v2.2.0';
const ASSETS = [
    './',
    './index.html',
    './style.css',
